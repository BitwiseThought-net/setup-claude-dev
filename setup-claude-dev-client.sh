...
# 2. System Dependencies (Installing gettext for envsubst)
sudo apt-get update
...
if ! command -v code &> /dev/null; then
    ...
    curl -fsSL https://microsoft.com | gpg --dearmor | sudo tee /etc/apt/keyrings/microsoft.gpg > /dev/null
    echo "deb [arch=amd64,arm64,armhf signed-by=/etc/apt/keyrings/microsoft.gpg] https://microsoft.com stable main" | sudo tee /etc/apt/sources.list.d/vscode.list > /dev/null
...
# envsubst takes the variables from our sourced config and injects them into the template
ALIAS_DEST="$HOME/.claude_aliases"
if [ -f "$TEMPLATE_ALIASES" ]; then
    echo "✍️  Generating $ALIAS_DEST from template..."
    # We export the variables so envsubst can see them
